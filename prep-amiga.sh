...
# MIKMOD
wget https://github.com/sezero/mikmod/archive/master.tar.gz -O mikmod.tgz
tar -xvf mikmod.tgz
mkdir -p mikmod-master/build
cd mikmod-master/build
rm -rf ../libmikmod/docs/*
touch ../libmikmod/docs/CMakeLists.txt
cmake ../libmikmod -DM68K_CPU="$M68K_CPU" -DM68K_FPU="$M68K_FPU" -DM68K_COMMON="${M68K_COMMON} -O3 -fno-exceptions -w -noixemul -DBIG_ENDIAN -DAMIGA -fpermissive -std=c++14"
...
./autogen.sh
SDL_LIBS='-lSDL -ldebug' SDL_CFLAGS="-L${SYSROOT}/usr/lib -I${SYSROOT}/usr/include/SDL -noixemul" CFLAGS="${M68K_CFLAGS}" CXXFLAGS="${M68K_CXXFLAGS}" ./configure --disable-sdltest --disable-shared --enable-static --enable-music-mod --host=${TARGET} --prefix="${SYSROOT}/usr"
...
cd SDL_ttf-SDL-1.2
./autogen.sh
LDFLAGS="-L${SYSROOT}/usr/lib" SDL_LIBS='-lSDL -ldebug -lpng16 -lz' SDL_CFLAGS="-L${SYSROOT}/usr/lib -I${SYSROOT}/usr/include/SDL -noixemul"  CFLAGS="${M68K_CFLAGS}" CXXFLAGS="${M68K_CXXFLAGS}" FT2_CFLAGS="-L${SYSROOT}/usr/lib -I${SYSROOT}/usr/include/freetype2" FT2_LIBS="-lfreetype -lzlib" ./configure --disable-shared --enable-static --host=${TARGET} --prefix=${SYSROOT}/usr
make -j$(getconf _NPROCESSORS_ONLN)
make install
cd ..
